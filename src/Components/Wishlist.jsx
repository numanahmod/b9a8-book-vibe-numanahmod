...
import { getStoredWishlist } from "./Utility/localStorageWishlist";


const Wishlist = () => {
    const books = useLoaderData();
    const [ listedWishlist, setListedWishlist] = useState([]);

     useEffect( () => {
        const storedBooksId = getStoredWishlist();


        if (books.length > 0 ){
            const booksRead = books.filter(book => storedBooksId.includes(book.Id) );

            setListedWishlist(booksRead);
    ...
    return (
        <>
        <p className="ml-12 mt-3"> Wishlist : {listedWishlist.length}</p>

        {
            listedWishlist.map(b => <section key={b.Id}>
